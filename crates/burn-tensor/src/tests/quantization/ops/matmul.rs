...
mod tests {
    ...
    fn test_matmul_simple_1() {
        let device = Default::default();
        // Quantized [[5.0, 14.0], [14.0, 50.0]]
        let data = TensorData::quantized(
            vec![13i8, 36, 36, 127],
            [2, 2],
            QuantizationStrategy::PerTensorSymmetricInt8(SymmetricQuantization::init(0.39370078)),
        );
        let tensor_1 = TestTensor::<2>::from_data(data, &device);
        // Quantized [[3.0, 4.0, 5.0], [0.0, 1.0, 2.0]]
        let data = TensorData::quantized(
            vec![76i8, 102, 127, 0, 25, 51],
            [2, 3],
    ...
    fn test_matmul_4_3() {
        ...
        let tensor_1 = TestTensor::<2>::from_data(data, &device);
        // Quantized [[0., 1., 2.], [4., 5., 6.], [8., 9., 10.], [12., 13., 14.]]
        let data = TensorData::quantized(
            vec![0i8, 9, 18, 36, 45, 54, 73, 82, 91, 109, 118, 127],
            [4, 3],
            QuantizationStrategy::PerTensorSymmetricInt8(SymmetricQuantization::init(0.11023622)),
        );
        let tensor_2 = TestTensor::<2>::from_data(data, &device);
    ...
    fn test_matmul_trivial() {
        ...
        let data = TensorData::quantized(
            vec![
                0i8, 8, 17, 25, 34, 42, 51, 59, 68, 76, 85, 93, 102, 110, 119, 127,
            ],
            [4, 4],
            QuantizationStrategy::PerTensorSymmetricInt8(SymmetricQuantization::init(0.11811024)),
        );
        let tensor_1 = TestTensor::<2>::from_data(data, &device);
